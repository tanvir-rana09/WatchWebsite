...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
	...
	return (
		<aside
			ref={sidebar}
			className={`absolute left-0 text-white top-0 pt-10 z-9999 flex h-screen w-[17rem] flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
				}`}
		>
			{/* <!-- SIDEBAR HEADER --> */}
												...
												<ul className="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6">
													<li>
														<NavLink
															to="/admin/products"
															className={({ isActive }) =>
																'group text-slate-400 relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ' +
																(isActive && '!text-white')
															}
														>
															Products
